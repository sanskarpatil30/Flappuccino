...
def main():
    ...
    while True:
        ...
        if dead:
            ...
            # "GAME OVER" title
            gameOverMsg = font_small.render("GAME OVER", True, (255, 80, 60))
            DISPLAY.blit(gameOverMsg, (DISPLAY.get_width()/2 - gameOverMsg.get_width()/2, 150))

            # final height score
            finalHeight = font_small.render("Height: " + str(height), True, (255, 255, 255))
            DISPLAY.blit(finalHeight, (DISPLAY.get_width()/2 - finalHeight.get_width()/2, 200))

            # final bean count
            finalBeans = font_small.render("Beans: " + str(beanCount), True, (255, 255, 255))
            DISPLAY.blit(finalBeans, (DISPLAY.get_width()/2 - finalBeans.get_width()/2, 240))

            # retry button centered
            retryX = DISPLAY.get_width()/2 - retry_button.get_width()/2
            retryY = 300
            DISPLAY.blit(retry_button, (retryX, retryY))
            retryMsg = font_small.render("RETRY", True, (0, 0, 0))
            DISPLAY.blit(retryMsg, (DISPLAY.get_width()/2 - retryMsg.get_width()/2, retryY + 4))

        height = round(-(player.position.y - startingHeight)/DISPLAY.get_height())

        # only move player when alive
        if not dead:
            ...
            if health <= 0:
                health = 0
                dead = True
                player.velocity.xy = 0, 0   # freeze player in place
        ...
        for bean in beans:
            ...
            if not dead and (checkCollisions(player.position.x, player.position.y, player.currentSprite.get_width(), player.currentSprite.get_height(), bean.position.x, bean.position.y, bean.sprite.get_width(), bean.sprite.get_height())):
        ...
        if not dead:
            for button in buttons:
                ...
                if clicked and checkCollisions(mouseX, mouseY, 3, 3, buttonX, buttonY, button.sprite.get_width(), button.sprite.get_height()):
        ...
        # retry logic — centered retry button when dead
        if dead and clicked:
            retryX = DISPLAY.get_width()/2 - retry_button.get_width()/2
            retryY = 300
            if checkCollisions(mouseX, mouseY, 3, 3, retryX, retryY, retry_button.get_width(), retry_button.get_height()):
                health = 100
